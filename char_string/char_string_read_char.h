#ifndef CHAR_STRING_READ_CHAR_H
#define CHAR_STRING_READ_CHAR_H

FUNCTION_INLINE char FUNC(read_char_char)(T *p)
{
	char c = 0;
...
FUNCTION_INLINE T* FUNC(read_char_size_flags)(T *p,
		const char *s,
		size_t size,
		unsigned int flags)
{
...
FUNCTION_INLINE T* FUNC(read_char)(T *p,
		const char *s)
{
	size_t size = 0;
	unsigned int flags = 0;

	return FUNC(read_char_size_flags)(p,
			s,
			size,
			flags);
}

FUNCTION_INLINE T* FUNC(read_char_size)(T *p,
		const char *s,
		size_t size)
{
	unsigned int flags = 0;
...
FUNCTION_INLINE T* FUNC(read_char_flags)(T *p,
		const char *s,
		unsigned int flags)
{
	size_t size = 0;
...
FUNCTION_INLINE T* FUNC(read_const_char)(T *p,
		const char *s,
		size_t s_size,
		unsigned int flags)
{
